...
def index():
    ...
    if request.method == "POST":
        ...
        sql = (
            "INSERT INTO hosts (ip_address, username, password) VALUES (%s, %s, %s, %s)"
        )
        val = (ip_add, username, password)
...
def memoryautocont():
    if request.method == "POST":
        id = request.form.get("id")
        url = (
            f"https://{ip_add}:8006/api2/json/nodes/{node_name}/lxc/{id}/status/current"
        )
        x = requests.get(url, verify=False, cookies=cookie)
        result = x.json()
        a = result["data"]["maxmem"]
...
def container():
    ...
    result = x.json()
    for x in range(len(result["data"])):
        result["data"][x]["disk"] = round(result["data"][x]["disk"] / 1073741824, 2)
        result["data"][x]["mem"] = round(result["data"][x]["mem"] / 1073741824, 2)
...
def containerview(id):
    ...
    result["data"]["maxdisk"] = round(result["data"]["maxdisk"] / 1073741824, 2)
